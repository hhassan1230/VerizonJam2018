...
namespace NVIDIA.Flex
{
    ...
    public class FlexAsset : ScriptableObject
    {
        ...
        protected void StoreAsset(FlexExt.Asset _asset)
        {
            ...
            if (_asset.numParticles > 0) FlexUtils.FastCopy(_asset.particles, m_particles);
            m_maxParticles = _asset.maxParticles;

            m_springIndices = new int[_asset.numSprings * 2];
            if (_asset.numSprings > 0) FlexUtils.FastCopy(_asset.springIndices, m_springIndices);
            m_springCoefficients = new float[_asset.numSprings];
            if (_asset.numSprings > 0) FlexUtils.FastCopy(_asset.springCoefficients, m_springCoefficients);
            m_springRestLengths = new float[_asset.numSprings];
            if (_asset.numSprings > 0) FlexUtils.FastCopy(_asset.springRestLengths, m_springRestLengths);

            m_shapeIndices = new int[_asset.numShapeIndices];
            if (_asset.numShapeIndices > 0) FlexUtils.FastCopy(_asset.shapeIndices, m_shapeIndices);
            m_shapeOffsets = new int[_asset.numShapes];
            if (_asset.numShapes > 0) FlexUtils.FastCopy(_asset.shapeOffsets, m_shapeOffsets);
            m_shapeCoefficients = new float[_asset.numShapes];
            if (_asset.numShapes > 0) FlexUtils.FastCopy(_asset.shapeCoefficients, m_shapeCoefficients);
            m_shapeCenters = new Vector3[_asset.numShapes];
            if (_asset.numShapes > 0) FlexUtils.FastCopy(_asset.shapeCenters, m_shapeCenters);

            m_triangleIndices = new int[_asset.numTriangles * 3];
            if (_asset.numTriangles > 0) FlexUtils.FastCopy(_asset.triangleIndices, m_triangleIndices);
